...
Of if you want the TOC to be a bullet- rather than numbered-list then use an
unordered list:

    * This unordered list will be replaced with a table of contents.
    {% raw %}{:toc}{% endraw %}

Adding `{% raw %}{:.no_toc}{% endraw %}` to a header, as I've done with the
Tables of Contents header above, excludes it from the table of contents:
